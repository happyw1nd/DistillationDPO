...
import numpy as np
import scipy
from utils.EMD import calc_EMD_with_sinkhorn_knopp
...
class EMD():
    def __init__(self, voxel_size=0.5, epsilon=0.001, max_iter=3000, tol=1e-4):
        self.emds = []
    ...
    def update(self, gt_pcd, pt_pcd):
        emd = calc_EMD_with_sinkhorn_knopp(np.array(pt_pcd.points), np.array(gt_pcd.points), voxel_size=self.voxel_size, epsilon=self.epsilon, max_iter=self.max_iter, tol=self.tol)
    ...
    def compute(self):
        emds_array = np.array(self.emds)
        return emds_array.mean(), emds_array.std()
