...
from utils.diff_completion_pipeline import DiffCompletion_DistillationDPO
...
@click.command()
@click.option('--path', '-p', type=str, default='datasets/SemanticKITTI/dataset/sequences/08', help='path to the scan sequence')
...
@click.option('--diff', '-d', type=str, help='trained diffusion model')
@click.option('--refine', '-r', type=str, help='refinement model')
@click.option('--use_refine', '-ur', type=bool, default=False, help='whether to use refine network')
@click.option('--save_name', '-sn', type=str, help='name of saved file')
def main(path, max_range, denoising_steps, cond_weight, diff, refine, use_refine, save_name): 
    ...
    for pose, scan_path in tqdm.tqdm(eval_list):
        ...
        # o3d.io.write_point_cloud(f"exp/metrics/pcd_gt.ply", pcd_gt)
        # o3d.io.write_point_cloud(f"exp/metrics/pcd_in.ply", pcd_in)
        # o3d.io.write_point_cloud(f"exp/metrics/pcd_pred.ply", pcd_pred)

        emd.update(pcd_gt, pcd_pred)
        avg_emd, _ = emd.compute()
        print(f'Mean emd: {avg_emd}')
